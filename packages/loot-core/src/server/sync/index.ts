...
function ensureGoldLotsTable(dataset: string) {
  ...
  const columns = db.runQuery<{
    name: string;
    notnull: number;
    type: string;
  }>('PRAGMA table_info(gold_lots)', [], true);
  if (
    columns.some(column => column.name !== 'id' && column.notnull) ||
    columns.some(
      column =>
        column.name === 'date' && column.type.toUpperCase() !== 'INTEGER',
    )
  ) {
    db.execQuery(`
